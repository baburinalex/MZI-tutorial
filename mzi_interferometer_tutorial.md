...
python mzi_tutorial.py     # построит все 8 рисунков в папку images/
```
...
### Синфазно vs дифференциально: что именно «видит» интерферометр

![Синфазное против дифференциального изменения n](images/fig8_common_vs_diff.png)

А что, если изменить $n$ **одинаково в обоих** волноводах делителя? Тогда разности постоянных распространения не возникает ($\delta = 0$), и из уравнений связанных мод синфазный сдвиг выносится как **общая фаза** — а она в $|T|^2$ сокращается. Деление остаётся 50:50, спектр MZI **не меняется вообще**.

На рисунке три случая при одинаковом по модулю $\Delta n$: опорный ($\Delta n = 0$), **синфазный** (оба волновода) и **дифференциальный** (один волновод). Синфазная кривая ложится точно на опорную — интерферометр её не замечает. Дифференциальная поднимает нули на обоих выходах.

$$\text{синфазно:}\ \ \delta = 0 \Rightarrow \text{нет эффекта}; \qquad \text{дифференциально:}\ \ \delta = \frac{\pi\,\Delta n}{\lambda} \neq 0 \Rightarrow \text{деление и ER «плывут»}.$$

Это фундаментальное свойство: **MZI реагирует на разность, а не на общий сдвиг.** Тот же принцип работает и для плеч — синфазный нагрев обоих плеч не двигает спектр, а дифференциальный двигает. Отсюда два практических следствия: балансный MZI устойчив к общему температурному дрейфу (common-mode rejection), а модуляторы делают **push-pull** — толкают фазу в плечах в противофазе, чтобы при том же напряжении получить вдвое больший дифференциальный сдвиг.

```python
from mzi_tutorial import (mzi_transmission, Lc_for_5050,
                          splitter_cross_power_symmetric, splitter_cross_power_detuned)

Lc, dn = Lc_for_5050(), 0.03
print("деление синфазно:       ", splitter_cross_power_symmetric(Lc, dn))   # 0.50 — не изменилось
print("деление дифференциально:", splitter_cross_power_detuned(Lc, dn))     # < 0.50
# синфазный спектр совпадает с опорным, дифференциальный — нет
```
